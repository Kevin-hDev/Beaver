...
**Sources** — `src/components/settings/memory-settings.tsx`, `system-prompt-settings.tsx`, `tools-settings.tsx`, `advanced-settings.tsx`, `advanced-settings-state.ts`, `file-access-settings.tsx`, `path-list-editor.tsx`, `session-workspace-settings.tsx`, `ollama-settings-section.tsx`, `vram-table.tsx` ; `src/components/agent-import/agent-import-settings.tsx` ; `src-tauri/src/services/agent_local/tools/tool_group_catalog.rs`, `memory_types.rs`, `memory_settings.rs` ; `src-tauri/src/commands/config.rs`, `commands/directory_access.rs`, `src-tauri/src/models/config.rs` ; `src/i18n/fr.json`
