...
4. Comment lire la quantification
...
**Le niveau de quantification** — noté Q4, Q5, Q8, f16. Un modèle est quantifié, c'est-à-dire compressé, pour tenir dans moins de mémoire, au prix d'une légère perte de qualité. C'est le levier le plus efficace : passer un modèle de 13 milliards de paramètres de la version non quantifiée à la version Q4 fait passer sa consommation de 28 Go à 8 Go.
...
|---|---|---|---|---|
| **3 milliards** | ~2 Go | ~2,5 Go | ~3,5 Go | ~6 Go |
| **7 milliards** | ~4,5 Go | ~5,5 Go | ~8 Go | ~16 Go |
| **13 milliards** | ~8 Go | ~9,5 Go | ~14 Go | ~28 Go |
| **30 milliards** | ~20 Go | ~23 Go | ~34 Go | ~68 Go |
| **70 milliards** | ~40 Go | ~48 Go | ~70 Go | ~140 Go |
...
|---|---|
| 8 Go | Modèles de 3 à 7 milliards, quantifiés en Q4 |
...
### Comment lire la quantification

| Notation | Ce que ça veut dire | Quand la choisir |
|---|---|---|
| **Q4_K_M** | Quantification forte | Le choix par défaut. Le meilleur compromis dans la quasi-totalité des cas |
| **Q5_K_M** | Quantification moyenne | Quand la mémoire le permet et que la qualité compte |
| **Q8_0** | Quantification légère | Rarement justifié : double la mémoire pour un gain à peine perceptible |
| **f16** | Aucune quantification | Réservé aux machines très bien dotées, ou à des mesures comparatives |

**Le conseil pratique à écrire sur le site** : entre un modèle plus grand fortement quantifié et un modèle plus petit peu quantifié, à mémoire égale, **le grand modèle quantifié gagne presque toujours**. Un modèle de 13 milliards en Q4 donne de meilleures réponses qu'un modèle de 7 milliards en Q8, pour la même mémoire.
...
L'écran de détail d'un modèle affiche ses caractéristiques réelles avant installation : taille du fichier, nombre de paramètres, longueur de contexte, niveau de quantification, architecture, et s'il s'agit d'un modèle à experts spécialisés.
...
1. **Prendre une quantification plus forte** du même modèle — Q8 vers Q4 divise la mémoire par deux.
...
> **À mémoire égale, préférez un grand modèle quantifié.**
> 13 milliards de paramètres en Q4 valent mieux que 7 milliards en Q8. La quantification coûte beaucoup moins en qualité que la réduction de taille.
...
|---|---|---|
| « Le modèle répond extrêmement lentement » | Il déborde sur la mémoire ordinaire | Quantification plus forte, ou modèle plus petit |
...
- Le **conseil « grand modèle quantifié plutôt que petit modèle peu quantifié »** est un consensus du domaine, pas une affirmation vérifiée dans le code de Beaver. À conserver, mais sans le présenter comme une mesure faite par l'équipe.
...
- **La table de mémoire est codée en dur dans l'interface** — confirmé (`src/components/settings/vram-table.tsx`), avec cinq tailles (`3B`, `7B`, `13B`, `30B`, `70B`) et quatre niveaux de quantification (`Q4_K_M`, `Q5_K_M`, `Q8_0`, `f16`). **Les vingt valeurs du tableau ci-dessus correspondent une à une au code.** La table ne s'adapte pas aux formats plus récents qui pourraient apparaître : sans conséquence aujourd'hui — ce sont des estimations — mais à surveiller.
