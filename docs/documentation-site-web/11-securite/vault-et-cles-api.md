...
- **`secrets.enc` figure dans la liste des fichiers protégés de l'application** : l'agent qui tente de le lire ou de le manipuler par une commande déclenche une alerte (`services/agent_local/permissions/sensitive_data.rs:1-6`, `:27-45`). La même liste couvre `config.json`, `agent-settings.json` et `configured-providers.json`.
