...
**Des versions anciennes de Beaver rangeaient certains jetons hors du coffre.** Au démarrage, Beaver supprime définitivement trois fichiers hérités : `oauth-providers/moonshot/credentials/kimi-code.json`, `oauth-providers/xai/auth.json` et `secrets.enc.bak-corrupted` (`services/security_cleanup.rs:26-28`, appelé depuis `app_build.rs:131`). Le nettoyage n'a lieu qu'une fois, un fichier marqueur `.security-hardening-v1` le mémorise (`security_cleanup.rs:4`, `:18-24`, `:30`). **À arbitrer avec l'équipe avant publication** : le dire montre que le problème est traité, mais révèle aussi qu'il a existé. Recommandation : le dire, dans la page de sécurité et pas ailleurs — un utilisateur qui trouve ces fichiers dans une sauvegarde ancienne a besoin de savoir qu'ils contenaient des jetons en clair et qu'il doit les révoquer.
...
3. **Faut-il mentionner le nettoyage des anciens fichiers de jetons ?** `security_cleanup.rs:26-28` supprime `oauth-providers/moonshot/credentials/kimi-code.json` et `oauth-providers/xai/auth.json`, deux fichiers qui contenaient des jetons hors du coffre dans des versions antérieures. Recommandation : le mentionner, parce qu'un utilisateur peut retrouver ces fichiers dans une sauvegarde ancienne et doit savoir qu'il faut révoquer les accès correspondants. Décision produit, pas technique.
