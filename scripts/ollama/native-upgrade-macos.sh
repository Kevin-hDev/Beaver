...
if [[ "${1:-}" != "--confirm-disposable-profile" || "${2:-}" != "--confirm-data-dir" || "$#" -ne 2 ]]; then
  echo "Usage: native-upgrade-macos.sh --confirm-disposable-profile --confirm-data-dir" >&2
  exit 2
fi
...
echo "Profile: disposable macOS account or VM only"
...
node "$SCRIPT_DIR/native-upgrade-proof.mjs" --data-dir "$DATA_DIR" --models-dir "$MODELS_DIR" --confirm-disposable-profile --confirm-data-dir --output "$PROOF_DIR/macos-before.json"
...
node "$SCRIPT_DIR/native-upgrade-proof.mjs" --data-dir "$DATA_DIR" --models-dir "$MODELS_DIR" --confirm-disposable-profile --confirm-data-dir --output "$PROOF_DIR/macos-after.json"
...
node "$SCRIPT_DIR/native-upgrade-proof.mjs" --data-dir "$DATA_DIR" --models-dir "$MODELS_DIR" --confirm-disposable-profile --confirm-data-dir --output "$PROOF_DIR/macos-recovered.json"
...
echo "macOS native upgrade proof metadata written under the local validation directory."
