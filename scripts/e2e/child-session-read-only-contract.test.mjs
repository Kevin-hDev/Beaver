...
test("every native E2E journey verifies chat_stream child read-only admission", () => {
  ...
  assert.match(
    wdioSource,
    /specs:\s*\[\[\s*childSessionReadOnlySpec,\s*extensionUiRuntimeProofSpec,\s*extensionUiAdvancedSpec,\s*extensionUiAcceptanceSpec,\s*extensionUiCleanupSpec,\s*extensionApiExpansionSpec,\s*extensionInstallBackgroundSpec,\s*journeySpec,?\s*\]\]/u,
  );
