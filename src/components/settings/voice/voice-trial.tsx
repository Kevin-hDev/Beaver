...
export function VoiceTrial({ language, languageMode, model, models, onModelChange }: {
  ...
  useEffect(() => () => {
    void dispatchVoiceAction({ action: "abandon-trial", trial_id: trialId })
      .catch(() => showToast(appI18n.t("errors.operationFailed"), "error"));
  }, [trialId]);
  const reportFailure = () => showToast(t("errors.operationFailed"), "error");
  const start = () => dispatchVoiceAction({ action: "start", destination: { kind: "trial", trial_id: trialId }, context_generation: Date.now(), language: resolveVoiceLanguage(language, i18n.resolvedLanguage ?? i18n.language, languageMode) }).catch(reportFailure);
  const validate = (operationId: string) => dispatchVoiceAction({ action: "validate", operation_id: operationId }).catch(reportFailure);
  return <section className="vset-trial">
    <h3>{t("voice.settings.try")}</h3>
    <div className="vset-trial-line">
      {operation ? <><VoiceSignal key={operation.id} level={operation.level} tick={operation.captureMs} /><button type="button" className="btn btn-sm btn-secondary" onClick={() => { setStoppedAt(performance.now()); setDelayMs(null); void validate(operation.id); }}>{t("voice.validate")}</button></>
    ...
    {!result && snapshot?.error && <p role="alert">{t(`voice.error.${snapshot.error.code}`)}</p>}
  </section>;
}
