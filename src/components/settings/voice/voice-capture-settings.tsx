...
import { voiceLanguageChoices } from "@/features/voice/voice-language-options";
...
export function VoiceCaptureSettings({ settings, devices, languages, languageMode, onSave, onRefresh }: {
  ...
  const languageOptions = useMemo(() => voiceLanguageChoices(languages, i18n.language, languageMode, {
    automatic: t("voice.language.automatic"),
    followInterface: t("voice.settings.followInterface"),
  }), [i18n.language, languageMode, languages, t]);
  const languageValue = languageMode === "automatic-only" || settings.language.kind === "automatic"
    ? languageMode === "automatic-only" ? "automatic" : "follow-interface"
  ...
  return <>
    ...
    <SettingsRow title={t("voice.settings.gain")} description={t("voice.settings.gainDescription")}><SettingsSelect value={settings.input_gain} options={["zero-db","three-db","six-db","nine-db"].map((value) => ({ value, label: t(`voice.settings.gainValues.${value}`) }))} onChange={(value) => onSave({ input_gain: value as VoiceSettings["input_gain"] })} /></SettingsRow>
    ...
    <SettingsRow title={t("voice.settings.language")}>
      {languageMode === "automatic-only"
        ? <span className="vset-language-mode">{languageOptions[0].label}</span>
        : <SettingsSelect value={languageValue} options={languageOptions} searchable onChange={(value) => onSave({ language: value === "follow-interface" ? { kind: "follow-interface" } : { kind: "language", value } })} />}
    </SettingsRow>
