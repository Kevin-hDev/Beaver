...
describe("useVoiceController", () => {
  beforeEach(() => {
    mocks.settings.mockResolvedValue({ enabled: true, model: "parakeet-tdt-v3", explanation_accepted: true, shortcut: null });
    mocks.catalog.mockResolvedValue([{ id: "parakeet", model: "parakeet-tdt-v3", installed: true }]);
  ...
  it("starts with the saved language instead of opening a per-message dialog", async () => {
    ...
    await waitFor(() => expect(mocks.dispatch).toHaveBeenCalledWith(expect.objectContaining({ action: "start", language: null })));
