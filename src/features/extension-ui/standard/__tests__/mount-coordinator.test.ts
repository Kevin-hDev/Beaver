...
describe("createMountCoordinator", () => {
  ...
  it("cleans a failed journal and continues with a healthy extension", async () => {
    let failAcknowledgement = true;
    vi.mocked(invoke).mockImplementation((command) => {
      if (command === "begin_extension_ui_load") return Promise.resolve([1, 2, 3]);
      if (command === "acknowledge_extension_ui_load" && failAcknowledgement) {
    ...
    const healthy = await coordinator.prepare("2:healthy", "com.example.healthy", 1);
    await healthy.commit();
    expect(commands()).toContain("abort_extension_ui_load");
    expect(commands().filter((command) => command === "begin_extension_ui_load"))
      .toHaveLength(2);
  });

  it("deduplicates the same contribution and continues after begin fails", async () => {
    const coordinator = createMountCoordinator();
    ...
    const queued = await coordinator.prepare("1:queued", "com.example.queued", 1);
    await queued.commit();
    expect(commands().filter((command) => command === "begin_extension_ui_load")).toHaveLength(3);
  });
});
