...
function measure(container: HTMLElement, axis: "x" | "y"): DragSlot[] {
  const box = container.getBoundingClientRect();
...
export function useDragReorder({ ids, axis, containerRef, onReorder }: DragReorderOptions) {
  ...
  const onPointerDown = useCallback((id: string, e: React.PointerEvent) => {
    ...
    const measured = measure(container, axis);
  ...
  const itemProps = useCallback((id: string) => ({
    ...
    style: transformStyle(offsets.get(id) ?? 0, axis),
  }), [axis, draggingId, offsets]);
...
function transformStyle(offset: number, axis: "x" | "y"): React.CSSProperties {
  if (offset === 0) return {};
