...
export type ActiveStreamItem =
  { kind: "thinking" } | { kind: "tools"; toolIndices: number[] } | null;
...
export function isPendingTool(tool: ToolActivity): boolean {
...
export function activeItemAfterToolResult(
  tools: ToolActivity[],
  finishedToolIndex: number,
): ActiveStreamItem {
  const pending = pendingToolIndices(tools);
  if (pending.length > 0) return toolItems(pending);
  if (finishedToolIndex >= 0 && finishedToolIndex < tools.length)
    return toolItems([finishedToolIndex]);
  return null;
}
