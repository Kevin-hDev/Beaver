...
pub(crate) fn safe_target_path(
    ...
    let target_canonical = if target.exists() {
        std::fs::canonicalize(&target).map_err(|e| {
            ::log::error!("[ollama-extract] canonicalize entry: {e}");
            "ollama-extract-error".to_string()
        })?
    } else {
        if let Some(parent) = target.parent() {
            std::fs::create_dir_all(parent).map_err(|e| {
                ::log::error!("[ollama-extract] mkdir: {e}");
                "ollama-extract-error".to_string()
            })?;
        }
        let parent_canonical =
            std::fs::canonicalize(target.parent().unwrap_or(dest)).map_err(|e| {
                ::log::error!("[ollama-extract] canonicalize parent: {e}");
                "ollama-extract-error".to_string()
            })?;
...
pub(crate) fn unpack_safe_symlink<R: Read>(
    ...
    let link = entry
        .link_name()
        .map_err(|e| {
            ::log::error!("[ollama-extract] symlink target: {e}");
            "ollama-extract-error".to_string()
        })?
    ...
    if let Ok(meta) = std::fs::symlink_metadata(target) {
        ...
        std::fs::remove_file(target).map_err(|e| {
            ::log::error!("[ollama-extract] remove existing symlink target: {e}");
            "ollama-extract-error".to_string()
        })?;
    }
    ...
    #[cfg(unix)]
    {
        std::os::unix::fs::symlink(&link, target).map_err(|e| {
            ::log::error!("[ollama-extract] symlink create: {e}");
            "ollama-extract-error".to_string()
        })?;
        Ok(())
    }
