...
pub(crate) fn safe_unpack_tar<R: Read>(
    ...
    let entries = archive.entries().map_err(|e| {
        ::log::error!("[ollama-extract] tar entries: {e}");
        "ollama-extract-error".to_string()
    })?;
    ...
    for entry_result in entries {
        ...
        let mut entry = entry_result.map_err(|e| {
            ::log::error!("[ollama-extract] tar entry: {e}");
            "ollama-extract-error".to_string()
        })?;
        let raw_path = entry
            .path()
            .map_err(|e| {
                ::log::error!("[ollama-extract] tar path: {e}");
                "ollama-extract-error".to_string()
            })?
            .into_owned();
...
fn unpack_entry<R: Read>(
    ...
    if entry_type.is_dir() {
        std::fs::create_dir_all(target).map_err(|e| {
            ::log::error!("[ollama-extract] mkdir entry: {e}");
            "ollama-extract-error".to_string()
        })?;
        return Ok(());
    }
    ...
    if let Some(parent) = target.parent() {
        std::fs::create_dir_all(parent).map_err(|e| {
            ::log::error!("[ollama-extract] mkdir parent: {e}");
            "ollama-extract-error".to_string()
        })?;
    }
    let mut output = std::fs::File::create(target).map_err(|e| {
        ::log::error!("[ollama-extract] create file: {e}");
        "ollama-extract-error".to_string()
    })?;
    ...
    loop {
        ...
        let read = entry.read(&mut buffer).map_err(|e| {
            ::log::error!("[ollama-extract] read entry: {e}");
            "ollama-extract-error".to_string()
        })?;
        if read == 0 {
            break;
        ...
        std::io::Write::write_all(&mut output, &buffer[..read]).map_err(|e| {
            ::log::error!("[ollama-extract] write entry: {e}");
            "ollama-extract-error".to_string()
        })?;
    }
