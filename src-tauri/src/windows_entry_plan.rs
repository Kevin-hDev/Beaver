...
const APPLICATION_MODULE_NAME: &str = "cl_go_dash_lib";
...
pub(crate) fn bootstrap_arguments(
    ...
    let mut result = Vec::with_capacity(MAX_FORWARD_ARGS + 1);
    result.push(OsString::from(format!(
        "--module={APPLICATION_MODULE_NAME}"
    )));
...
pub(crate) fn stage_application_module(root: &Path) -> Result<PathBuf, ()> {
    ...
    replace_module(&source, &destination, root)?;
...
fn replace_module(source: &Path, destination: &Path, root: &Path) -> Result<(), ()> {
    ...
    let temporary = root.join("cl_go_dash_lib.dll.tmp");
