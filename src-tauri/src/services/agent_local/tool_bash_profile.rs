...
impl ShellProfile {
    fn wrap(&self, command: &str) -> Zeroizing<String> {
        let mut wrapped = Zeroizing::new(String::with_capacity(
            self.script.len() + command.len() + 16,
        ));
        wrapped.push_str(&self.script);
        wrapped.push_str("\neval ");
        push_single_quoted(&mut wrapped, command);
        wrapped
    }
}
...
pub async fn prepare(
    ...
    let Some(cell) = profile_cell(owner_session_id) else {
    ...
    profile
        .as_ref()
        .map(|profile| profile.wrap(command))
        .unwrap_or_else(|| Zeroizing::new(command.to_string()))
}
...
fn snapshot_script(shell: &str, marker: &str) -> Option<String> {
    ...
    } else if name == "sh" || name == "dash" || name == "ksh" {
        Some(format!(
