...
async fn captures_applies_and_cleans_temporary_branch() {
    ...
    assert_eq!(
        applied.status,
        super::types_subagent_change::SubagentChangeStatus::Applied
    );
    assert_eq!(
...
async fn correction_replays_pending_change_from_new_parent_head() {
    ...
    let next = subagent_worktree::create_for_execution(repo.path(), &child.id, &next_execution)
        .await
        .expect("next worktree");
    subagent_git_run::seed_pending(repo.path(), &child.id, &next_execution, &next)
        .await
        .expect("replay pending");
    assert_eq!(
        std::fs::read_to_string(next.join("child.txt")).unwrap(),
        "first\n"
    );
    assert_eq!(
        std::fs::read_to_string(next.join("parent.txt")).unwrap(),
        "advanced\n"
    );
    let migrated = super::subagent_change_store::load(&child.id)
        .await
        .expect("migrated");
