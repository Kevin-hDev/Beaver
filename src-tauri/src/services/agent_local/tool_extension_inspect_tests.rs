use super::{
    decision, discover_with_refresh, ids, indexed_active_record_is_available, Decision,
};
use crate::services::agent_local::{
    extension_session_plugins::refresh_active_with_catalog,
    extension_session_state::ExtensionSessionState,
    extension_tool_selection::PluginDescriptor,
};
...
fn rejects_duplicate_and_more_than_four_identifiers() {
    ...
    assert!(ids(&json!({"ids":["example.a","example.b","example.c","example.d","example.e"]})).is_err());
}

#[test]
fn active_approved_records_missing_from_the_index_are_unavailable() {
    assert!(!indexed_active_record_is_available(Some((true, true)), false));
    assert!(indexed_active_record_is_available(Some((true, true)), true));
    assert!(indexed_active_record_is_available(Some((false, true)), false));
}
...
async fn inspection_event_is_bounded_and_refresh_correlates_its_uuid() {
    ...
    let result = super::execute(&json!({"ids": ["example.a"]}), &session.id, Some(&request_id)).await;
    ...
    let pending = super::super::stream_diagnostics::pending_extension_inspections(&session.id, &request_id).await;
...
async fn invalid_inspection_records_no_untrusted_identifier() {
    ...
    .unwrap();
    let request_id = super::super::stream_diagnostics::start_request(&session.id, 1).await;
    let result = super::execute(&json!({"ids": ["not valid!"]}), &session.id, Some(&request_id)).await;
