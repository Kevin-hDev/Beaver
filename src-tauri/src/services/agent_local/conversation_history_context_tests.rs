...
async fn next_turn_rebuilds_prior_text_image_and_skill_context_from_rust_authorities() {
    ...
    let first = conversation_admission::new_turn_with_key(
        &session.id,
        first,
        target("model-a"),
        &KEY,
    ...
    let second = conversation_input::resolve_with_key(
        NewUserTurnInput { content: "second visible".into(), files: vec![], skills: vec![] },
        &KEY,
    )
    .await
    .unwrap();
    let second = conversation_admission::new_turn_with_key(
        &session.id,
        second,
        target("model-a"),
        &KEY,
    ...
    assert_eq!(persisted.messages[0].skill_ids.as_deref(), Some(&[fixture.skill_id.clone()][..]));
...
async fn unavailable_historical_file_or_skill_blocks_with_one_generic_error() {
    for remove_skill in [false, true] {
        let session = create_session().await;
        let fixture = install_context_fixture(if remove_skill { "missing-skill" } else { "missing-file" });
        ...
        conversation_admission::new_turn_with_key(&session.id, input, target("model-a"), &KEY)
            .await
            .unwrap();
        if remove_skill { fs::remove_dir_all(&fixture.skill_root).unwrap(); }
        else { fs::remove_file(&fixture.text_path).unwrap(); }

        let error = conversation_history::load_for_target_with_key(
            &session.id,
            &target("model-a"),
            &KEY,
        )
...
async fn unavailable_prior_context_blocks_admission_before_mutating_the_session() {
    ...
    let mut durable = super::super::session_store::get(&session.id).await.unwrap();
    durable.messages.push(message(
        "00000000-0000-4000-8000-000000000098", &admitted.turn_id, "assistant", "answer",
    ));
    super::super::session_store::save(&durable).await.unwrap();
    ...
    let next = conversation_input::resolve_with_key(
        NewUserTurnInput { content: "next".into(), files: vec![], skills: vec![] }, &KEY,
    ).await.unwrap();

    let error = conversation_admission::new_turn_with_key(
        &session.id, next, target("model-a"), &KEY,
    ).await.expect_err("unavailable prior context blocks admission");
...
impl ContextFixture {
    fn input(&self, content: &str) -> NewUserTurnInput {
        ...
        let registered = crate::services::attachment_access::register_paths(
            std::slice::from_ref(&raw), &KEY, |_| true,
        ).unwrap();
        NewUserTurnInput {
            content: content.into(),
            files: vec![
                TurnAttachmentInput {
                    name: "notes.txt".into(), path: registered[0].path.clone(),
                    mime_type: "text/plain".into(), size: registered[0].size,
                    thumbnail: None, access_grant: Some(registered[0].access_grant.clone()),
                },
                TurnAttachmentInput {
                    name: "image.png".into(), path: String::new(), mime_type: "image/png".into(),
                    size: 12, thumbnail: Some("data:image/png;base64,iVBORw0KGgoAAAAA".into()),
                    access_grant: None,
                },
            ],
            skills: vec![SkillReference { id: self.skill_id.clone(), name: None }],
        }
    }

    fn cleanup(&self) { let _ = fs::remove_dir_all(&self.root); let _ = fs::remove_dir_all(&self.skill_root); }
}
...
fn install_context_fixture(label: &str) -> ContextFixture {
    ...
    fs::write(skill_root.join("SKILL.md"), "---\nname: Historical\ndescription: fixture\n---\ntrusted historical body").unwrap();
    let canonical = skill_root.canonicalize().unwrap();
    let skill_id = super::super::skill_catalog::entries().unwrap().into_iter()
        .find(|entry| entry.bundle_root == canonical).expect("installed skill").info.id;
    ContextFixture { root, text_path, skill_root, skill_id }
}
