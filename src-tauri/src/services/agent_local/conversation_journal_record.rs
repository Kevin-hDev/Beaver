...
pub(super) fn from_message(message: &ChatMessage, id: String, turn_id: &str, request_id: &str) -> Result<AgentMessage, String> {
    let tool_calls = message.tool_calls.as_ref().map(|calls| calls.iter().map(|call| Ok(ToolCallRequest {
        id: call.id.clone().ok_or_else(error)?, extra_content: call.extra_content.clone(),
        function: ToolCallRequestFunction { name: call.function.name.clone(), arguments: call.function.arguments.clone() },
