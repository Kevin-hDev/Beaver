...
fn maps_extension_loader_failures_to_the_existing_generic_skill_errors() {
    ...
    assert_eq!(
        map_error(ResourceLoadError::InvalidId),
        SkillLoadError::InvalidId
    );
    assert_eq!(
        map_error(ResourceLoadError::TooLarge),
        SkillLoadError::Unavailable
    );
    assert_eq!(
        map_error(ResourceLoadError::NotFound),
        SkillLoadError::NotFound
    );
    assert_eq!(
        map_error(ResourceLoadError::Unavailable),
        SkillLoadError::Unavailable
    );
}
...
fn enriched_extension_skill_is_bounded_after_provenance_is_added() {
    ...
    assert_eq!(
        exact.content.len(),
        super::skill_limits::MAX_RESOLVED_SKILL_BYTES
    );

    let oversized_body =
        "x".repeat(super::skill_limits::MAX_RESOLVED_SKILL_BYTES - prefix.len() + 1);
    assert!(matches!(
...
async fn ordinary_skill_ids_still_delegate_to_the_global_catalog() {
    ...
    assert!(matches!(
        result,
        Err(super::tool_skill_loader::SkillLoadError::NotFound)
    ));
}
