...
async fn sixty_four_corrections_are_allowed_and_sixty_fifth_is_rejected() {
    ...
    session_store::save(&saved)
        .await
        .expect("save over-cap queue");
    let result = subagent_instruction_delivery::drain(&child.id, &mut Vec::new()).await;
    let still_queued = session_store::get(&child.id)
        .await
        .expect("load rejected queue");
    ...
    session_store::save(&cleared)
        .await
        .expect("clear test queue");
    let tool_result = super::tool_subagent_message::run(
        &serde_json::json!({"subagent_id": child.id, "prompt": "correction 64"}),
        &parent.id,
    )
    .await;
    let final_child = session_store::get(&child.id)
        .await
        .expect("load final child");
