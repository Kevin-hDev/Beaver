use super::{
    session_store, subagent_git_actions, subagent_git_run, subagent_registry, subagent_working_dir,
};
...
async fn empty_directory_can_be_prepared_for_a_coder() {
    ...
    let worktree =
        super::subagent_directory_workspace::create(project.path(), &child_id, &execution_id)
            .await
...
async fn directory_conflict_preserves_user_files_and_can_be_discarded() {
    ...
    let mut parent =
        session_store::create_full("Parent", "model", "provider", false, Some("project".into()))
            .await
            .expect("parent");
    parent.working_dir = project.path().to_string_lossy().to_string();
    session_store::save(&parent).await.expect("save parent");
    let mut child =
        session_store::create_full("Child", "model", "provider", false, Some("project".into()))
            .await
            .expect("child");
    ...
    session_store::save(&running_child)
        .await
        .expect("save worktree path");
    subagent_working_dir::cleanup_owned(&child.id, &run.execution_id, prepared.worktree_path())
        .await;
    let (_, patch, _) =
        subagent_git_actions::inspect(project.path(), &parent.id, &child.id, &change.id)
            .await
            .expect("inspect");
    ...
    let conflict = super::subagent_change_store::load(&child.id)
        .await
        .expect("conflict");
    assert_eq!(
        conflict.status,
    ...
    subagent_git_actions::discard(project.path(), &parent.id, &child.id, &change.id)
        ...
        .expect("discard");
    let repository =
        super::subagent_directory_workspace::repository_path(&child.id, &run.execution_id)
            .expect("repository");
