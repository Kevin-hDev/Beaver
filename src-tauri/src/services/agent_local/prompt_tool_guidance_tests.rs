...
fn discovery_tools_have_distinct_scopes() {
    for prompt in prompts() {
        ...
        assert!(prompt.contains("Inspect directly with inspect_extensions using 1–4 known exact IDs"));
        assert!(prompt.contains("list_extensions only for a complete view, descriptions, or counts"));
...
fn discovery_guidance_disappears_with_its_unavailable_tool() {
    for prompt in prompts() {
        ...
        assert!(!filtered.contains("Inspect directly with inspect_extensions using 1–4 known exact IDs"));
        assert!(!filtered.contains("list_extensions only for a complete view, descriptions, or counts"));
