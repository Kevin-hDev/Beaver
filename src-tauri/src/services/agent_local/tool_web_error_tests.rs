...
fn search_configuration_rate_limit_and_timeout_are_distinct() {
    ...
    let limited = search(unstructured(
        "Brave: limite de requêtes atteinte (HTTP 429)",
    ));
...
fn every_declared_searxng_code_has_an_explicit_tool_classification() {
    ...
    let expected = [
        ...
        (
            error_codes::SEARCH_INVALID_RESPONSE,
    ...
    for (code, expected_tool_code) in expected {
        let failure =
            crate::services::search::finish_search(false, false, Vec::new(), Err(code.to_string()))
                .unwrap_err();
