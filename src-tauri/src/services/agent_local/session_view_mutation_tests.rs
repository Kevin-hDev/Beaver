...
async fn metadata_patch_preserves_the_canonical_envelope_byte_for_byte() {
    ...
    let restored = super::session_store::get(&session.id)
        .await
...
async fn edit_input_changes_only_the_targeted_user_content() {
    let mut session =
        super::session_store::create_full("Edit fixture", "fixture-model", "ollama", false, None)
            .await
            .expect("create session");
