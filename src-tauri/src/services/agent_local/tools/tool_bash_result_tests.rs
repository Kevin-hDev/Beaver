...
fn timeout_preserves_process_stderr_and_adds_the_completion_cause() {
    ...
    let code = completion_exit_code(Some(CompletionKind::TimedOut), &mut stdout, &mut stderr);
...
fn cancellation_and_internal_failure_keep_distinct_messages() {
    ...
    completion_exit_code(Some(CompletionKind::Cancelled), &mut stdout, &mut cancelled);
