...
fn resolves_all_admitted_files_before_attaching_them() {
    ...
    result
        .set_pending_artifacts(vec![pending("one.txt"), pending("two.txt")])
        .unwrap();
...
fn cancellation_before_reading_keeps_no_partial_artifact() {
    ...
    let mut result = ToolResult::ok("done");
    result
        .set_pending_artifacts(vec![pending("one.txt")])
        .unwrap();
...
fn malformed_or_oversized_candidates_use_translated_extension_codes() {
    ...
    invalid
        .set_pending_artifacts(vec![pending("../outside.txt")])
        .unwrap();
    ...
    too_large
        .set_pending_artifacts(vec![pending("large.bin")])
        .unwrap();
