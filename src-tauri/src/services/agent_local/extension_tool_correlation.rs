...
pub async fn pending_extension_inspections(
    session_id: &str,
    request_id: &str,
    ...
    run.events[after..]
        ...
        .filter(|diagnostic| {
            diagnostic.origin == ExtensionDiagnosticOrigin::Inspection.as_str()
                && diagnostic
                    .correlation_id
                    .as_deref()
...
pub async fn record_extension_refreshes(
    ...
    extension.related_inspection_ids = related_inspection_ids;
    let _ =
