...
use super::{
    session_store, subagent_completion, subagent_registry, subagent_status, tool_delegate_child,
};
...
async fn message_between_terminal_save_and_registry_completion_is_never_stranded() {
    ...
    let restart = tokio::spawn(async move {
        ...
        let registered_run =
            subagent_registry::register(&restart_parent, &restart_child, CancellationToken::new())
                .await
    ...
    assert_eq!(
        saved.subagent_prompt.as_deref(),
        Some(accepted_prompt.as_str())
    );
    assert!(saved
        .messages
        .iter()
        .any(|message| { message.role == "user" && message.content == accepted_prompt }));
    assert!(subagent_registry::active_children_for_parent(&parent.id)
        .await
        .contains(&child.id));
    assert_eq!(
        super::subagent_hidden_reports::peek_reports(&parent.id)
            .await
            .len(),
        1
    );
