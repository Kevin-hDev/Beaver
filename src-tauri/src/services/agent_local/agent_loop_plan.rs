...
mod tests {
    ...
    fn replaces_existing_plan_correction() {
        let mut messages = vec![
            ChatMessage::system("<plan_mode_backend_correction>old</plan_mode_backend_correction>".into()),
            ChatMessage::user("hello".into()),
        ];
        replace_correction(
            &mut messages,
            ChatMessage::system("<plan_mode_backend_correction>new</plan_mode_backend_correction>".into()),
        );
