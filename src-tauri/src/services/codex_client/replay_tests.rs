...
fn collector_attaches_bounded_codex_state_to_first_tool() {
    ...
    let function = serde_json::json!({"type":"function_call","call_id":"call_1","name":"read_file","arguments":"{\"path\":\"a\"}"});
    ...
    assert_eq!(
        result.tool_call_extra_content[0]
            .as_ref()
            .unwrap()
            .pointer(CODEX_ITEMS_PATH)
            .unwrap(),
        &serde_json::json!([reasoning, function])
    );
}
...
fn replay_rejects_state_for_another_tool_call() {
    let message = ChatMessage::assistant(
        ...
        Some(vec![ToolCallOllama {
            ...
            extra_content: Some(
                serde_json::json!({"codex":{"output_items":[{"type":"function_call","call_id":"call_other","name":"read_file","arguments":"{}"}]}}),
            ),
