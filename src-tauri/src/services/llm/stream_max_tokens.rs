...
pub async fn resolve(
    ...
    let (context_window, model_limit, model_default) =
        select_sources(local, runtime_limits, registered);
    choose(
        requested,
        model_limit,
        model_default,
...
fn choose(
    ...
    let candidate = match (requested, auto_max_tokens) {
        (Some(value), _) => Some(model_limit.map_or(value, |limit| value.min(limit))),
        (None, false) => None,
        (None, true) => model_default
            .or(model_limit)
            .or_else(|| provider_fallback.filter(|limit| *limit > 0)),
    };
