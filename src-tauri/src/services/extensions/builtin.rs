...
pub fn records() -> Result<Vec<ExtensionRecord>, String> {
    let catalog: BuiltinCatalog = serde_json::from_str(CATALOG)
        .map_err(|_| error_codes::BUILTIN_CATALOG_INVALID.to_string())?;
    let records = catalog
        .plugins
        .into_iter()
...
pub fn resolve_entry(host_directory: &Path, record: &ExtensionRecord) -> Result<PathBuf, String> {
    if record.kind != ExtensionKind::Builtin {
        return Err(error_codes::BUILTIN_PLUGIN_INVALID.to_string());
    }
    ...
    let plugin_root = root
        ...
        .canonicalize()
        .map_err(|_| error_codes::BUILTIN_CATALOG_UNAVAILABLE.to_string())?;
    let main = record
        .manifest
        .main
        .as_deref()
        .ok_or_else(|| error_codes::BUILTIN_ENTRY_MISSING.to_string())?;
    let entry = root
        .join(main)
        .canonicalize()
        .map_err(|_| error_codes::BUILTIN_ENTRY_UNAVAILABLE.to_string())?;
    if !entry.starts_with(&plugin_root) || !entry.is_file() {
        return Err(error_codes::BUILTIN_ENTRY_INVALID.to_string());
    }
    Ok(entry)
}
