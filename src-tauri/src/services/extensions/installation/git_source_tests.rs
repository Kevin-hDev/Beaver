...
impl super::install_signal::InstallSignal for MovingRemote {
    fn downloaded(&self, _bytes: u64) -> bool {
...
fn git_volume_replay_keeps_the_pinned_commit_when_remote_head_moves() {
    for abbreviated in [false, true] {
        ...
        if abbreviated {
            commit_payload(&remote, b"new tip before resolving the old prefix");
        }
        ...
        let source = GitSource {
            locator: "https://example.invalid/repo.git".into(),
            clone_url: url::Url::from_file_path(&remote).unwrap().to_string(),
            reference: abbreviated.then(|| expected.chars().take(8).collect()),
        };
